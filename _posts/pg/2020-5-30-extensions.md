...
PostgreSQL（以下简称 PG）是当下最流行的数据库之一，很多数据库是基于 PG 开发的。

例如 YugabyteDB，虽替换了存储引擎，但还是基于 PG 的 SQL 层进行改造，以降低开发量。由于需要修改源码，这一类的实现原理比较好理解。

有些数据库中间件是基于 PG 开发的，例如 Citus 在 PG 之上实现分布式数据库，把 PG 作为存储节点，只需在计算层再实现分布式计算，把单机计算的部分下推给 PG。

本文要介绍的几款数据库就很不一样，它们是以 PG 扩展的形式开发的：

* PostGIS 是空间数据库
* TimescaleDB 是时序数据库
* AgensGraph 是图数据库

从直觉上来说，这些数据库一般归为 NoSQL 数据库，它们的解析器、优化器、执行器、存储引擎等整个流程都与 RDBMS 不一样。那么它们是如何以 PG 扩展的形式开发的呢？
...
* 可以直接支持 SQL，形成多模数据库，用户只用部署一个数据库实例
* PG 更新版本之后，用户可以及时地享受 PG 的新特性
* PG 更新版本之后，开发人员维护与 PG 的兼容性也轻松很多
* 遵循 PG 规范，降低用户的学习成本
...
其中 FDW 一般是把其他数据库当作 PG 的数据源，本文的几款数据库不属于 FDW 扩展。
...
* 几何形状基于平面，对应的类型是 geometry，例如 geometry(POINT)。因为基于平面计算，geometry 的效率更高。如果空间仅局限于一个市，或更小的区域，建议用几何。
* 地理形状基于球体，对应的类型是 geography，例如 geography(POINT)。它存储经纬度，但是因为计算更复杂，所以可用的函数更少。

PostGIS 对 geometry 和 geography 都支持两种格式：Well-Known Text (WKT) 和 Well-Known Binary (WKB)。其中 WKT 易于阅读，WKB 效率更高。生产环境一般用 WKB。两种对象可以相互转换，例如函数 `ST_AsBinary` 把 WKT 转换为 WKB。
...
PG 还有原生的语法进行类型转换，例如把文本转为 geometry：

```sql
SELECT 'SRID=4326;POINT(0 0)'::geometry;
```

因为 PG 支持扩展数据类型，所以 PostGIS 可以方便地在 PG 上扩展 geometry 和 geography 类型。

### 空间索引

对多边形进行计算，计算量非常大，但如果把它们简化为矩形，计算就快很多。即使是最复杂的多边形和线串（LineString) 也可以用一个简单的 bounding box（边界框）来表示。

空间索引的基本思路是存储对象的 bounding box，二维图形的 bounding box 是一个矩形，三维是一个长方体。检索时先用 bounding box 近似计算，过滤大部分对象，再进行精确计算，这样大大减少了读取的数据量。例如，“多边形内包含了哪些线段” 可以先近似为 “多边形的边界框内包含了哪些线段的边界框”。

计算二维图形的 bounding box：

![bounding-box](/media/pg/bounding-box.png)

BTree 只能搜索一维数据，而空间索引需要检索多维数据。RTree 和 BTree 一样也是把数据集迭代地分割成更小的数据集，最终形成一棵平衡树。不同的是它是 BTree 在多维空间的扩展，可以分割多维空间。
...
除函数外，还支持操作符进行近似计算。例如使用 `&&` 代替 `ST_Intersects`，可以进行近似查询，但它是纯索引查询，效率高很多。例如 `geom1 && geom2` 检查相交。同理，用 `=` 比较两个几何体的 bounding box 是否完全相等。

### 查询语言

空间数据库与关系型数据库的差异不算大，它仍然可以用关系型数据库来存储，用 SQL 查询。
...
* 绝大多数写入都是追加，极少需要更新
* 越新的数据查询越频繁
* 大多以时间维度查询，也可以按 metric 聚合

### 存储

由于时序数据库的数据量巨大，如果用传统的 BTree 存储，当单表的数据量达到 TB 级别后写入性能必然严重下降。

TimescaleDB 通过 hypertable 存储数据。Hypertable 是 TimescaleDB 定义的一种抽象，它在逻辑上是一张表，但物理实现是多个 chunk，每个 chunk 是一张小表。每隔一段时间，TimescaleDB 就会自动创建新 chunk，后续的写入写到新 chunk 里，新 chunk 常驻内存。这样，TimescaleDB 通过把表拆分成多个 BTree 保证了写入性能。

TimescaleDB 官网上贴出了数据导入的性能对比：

![timescale-vs-postgres-insert](/media/pg/timescale-vs-postgres-insert.jpg)

建 hypertable 的步骤是先建一个普通表，然后用 `create_hypertable(table_name, key)` 函数把它转换为 hypertable，同时指定各种参数。其中参数 `chunk_time_interval` 是分割 chunk 的时间间隔，每隔 `chunk_time_interval` 就创建一个新的 chunk。
...
PG 允许表之间有继承。TimescaleDB 实现 chunk 的方法是表继承，每个 chunk 其实是一张子表，继承于父表 hypertable。

除了 DML 外，hypertable 的 DDL 也是对用户透明的。例如对 hypertable 的 `ALTER` 语句都被传播给它的每一个 chunk。
...
为了适应时序的场景，一些时序数据库会定制查询语言，例如 InfluxDB 使用 Flux。

TimescaleDB 借助窗口函数实现时序数据库的一些功能，例如：
...
AgensGraph-ext 通过 SQL 函数 `cypher()` 实现了在 SQL 中嵌入 Cypher：把 graph name 作为第一个参数；把 Cypher 语句像字符串一样用 `$$` 包起来作为第二个参数，AgensGraph-ext 内部解析 Cypher 语句。
...
以上分析了 PostGIS、TimescaleDB、AgensGraph 的大体实现方式，窥见了 PG 强大的扩展能力。其中必不可少的能力包括：

* 可以动态地扩展数据类型
* 可以动态地扩展 SQL 函数

但是同时看到，PG 扩展的形态虽然带来很多便利，但也限制了这些数据库的能力：
